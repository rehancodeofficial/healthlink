              ...
              <div className="divide-y divide-[var(--border)]">
                {waitingPatients.length > 0 ? (
                  waitingPatients.map((apt) => (
                    <div
                      key={apt.id}
                      className="p-4 flex items-center justify-between hover:bg-[var(--bg-main)]/50 transition-all"
                    >
                      ...
                      <button
                        onClick={() =>
                          navigate(`/doctor/video-consultation?appointmentId=${apt.id}`)
                        }
...
function AlertCard({ icon, label, count, color, bg, onClick }) {
  return (
    <div
      onClick={onClick}
      className={`p-4 rounded-3xl ${bg} flex items-center justify-between cursor-pointer hover:brightness-95 transition-all border border-transparent hover:border-white/20`}
    >
      <div className="flex items-center gap-4">
        <div
          className={`h-10 w-10 rounded-2xl bg-white flex items-center justify-center text-lg ${color} shadow-sm`}
        >
          {icon}
        </div>
        <div>
          <p className={`text-[10px] font-black uppercase tracking-widest ${color}`}>{label}</p>
          <p className="text-lg font-black text-[var(--text-main)]">{count} Items</p>
        </div>
      </div>
