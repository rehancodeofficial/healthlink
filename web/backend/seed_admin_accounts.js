  ...
  for (const account of accounts) {
    ...
    if (!authUser) {
      ...
      if (existingPrismaUser) {
        console.log(`  [DB] Found existing user in DB with ID: ${existingPrismaUser.id}. Checking Auth for this ID...`);
        const { data: userData, error: userError } = await supabase.auth.admin.getUserById(existingPrismaUser.id);
        if (userData && userData.user) {
          console.log(`  [Auth] Found user by ID in Auth!`);
          authUser = userData.user;
        } else {
          console.log(`  [Auth] User not found in Auth by ID ${existingPrismaUser.id}. Error: ${userError?.message}`);
        }
      }
    }

    if (authUser) {
      console.log(`  [Auth] User already exists, updating password + confirming...`);
