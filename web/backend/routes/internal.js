...
router.post("/fix-schema", async (req, res) => {
  try {
    ...
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "DoctorProfile" ADD COLUMN IF NOT EXISTS "emergencyContact" TEXT`,
    );
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "DoctorProfile" ADD COLUMN IF NOT EXISTS "emergencyContactName" TEXT`,
    );
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "DoctorProfile" ADD COLUMN IF NOT EXISTS "emergencyContactEmail" TEXT`,
    );
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "DoctorProfile" ADD COLUMN IF NOT EXISTS "customProfession" TEXT`,
    );
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "DoctorProfile" ADD COLUMN IF NOT EXISTS "primaryContact" TEXT`,
    );

    // Add missing cols to PatientProfile if any
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "PatientProfile" ADD COLUMN IF NOT EXISTS "emergencyContactName" TEXT`,
    );
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "PatientProfile" ADD COLUMN IF NOT EXISTS "emergencyContactEmail" TEXT`,
    );
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "PatientProfile" ADD COLUMN IF NOT EXISTS "heightUnit" TEXT DEFAULT 'cm'`,
    );
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "PatientProfile" ADD COLUMN IF NOT EXISTS "weightUnit" TEXT DEFAULT 'kg'`,
    );
...
router.get("/test-gemini", async (req, res) => {
  try {
    const testResult = await geminiService.generateAIResponse(
...
router.get("/debug-env", (req, res) => {
  res.json({
    ...
    geminiKeyLength: process.env.GEMINI_API_KEY
      ...
      : 0,
    googleKeyLength: process.env.GOOGLE_API_KEY
